...
import { connectFoodCategory, confirmEmail, getUserID, insertUserSql, getPreferToUserID, insertUserMissionSql, getUsermissionID, getReviewByUserIdAtFirst, getReviewByUserId, getMissionByUserIdAtFirst, getMissionByUserId } from "./user.sql.js";
...
export const getPreviewMission = async (cursorId, size, userId) => {
    try {
        ...
        if(cursorId == "undefined" || typeof cursorId == "undefined" || cursorId == null){
            const [missions] = await pool.query(getMissionByUserIdAtFirst, [parseInt(userId), parseInt(size)]);
            conn.release();
            return missions;
    
        }else{
            const [missions] = await pool.query(getMissionByUserId, [parseInt(userId), parseInt(cursorId), parseInt(size)]);
            conn.release();
            return missions;    
        }
    } catch (err) {
